...
%  2. Sign and date section 0 with your advisor.
%  3. Optionally post to OSF for an independent timestamp (free, ~10 minutes).
...
{Version 0.1 (draft for advisor review) \textbullet\ \today}
\end{center}
...
Author & Liam Crettol \\
Advisor & \rule{5cm}{0.4pt} \\
Advisor signature & \rule{5cm}{0.4pt} \\
Independent timestamp & OSF DOI: \rule{4cm}{0.4pt} \quad (optional but recommended) \\
Amendments to date & 0 \\
\bottomrule
\end{tabular}

\vspace{8pt}
\noindent The signature above attests that this protocol was fixed before any
connectivity, corridor, current-flow, protected-area overlap, or prioritisation
result had been generated or viewed.
...
\dec{Confirm you accept the H1 revision. If you prefer to keep the original H1,
it must be demoted from a hypothesis to a descriptive attribution analysis, and
the word ``hypothesis'' removed from it.}
...
\dec{Confirm the \SI{100}{\kilo\metre} buffer and the 120-hour compute trigger.
The trigger should reflect hardware you actually have, so measure a pilot run
first (timeline block 5) and amend this value before locking if needed.}
...
\dec{Paste the full WKT string into the box below when you set up the ArcGIS Pro
project, so the projection is unambiguous and reproducible.}

\begin{center}
\fbox{\parbox{0.92\textwidth}{\vspace{1.5cm}\centering\emph{full CRS WKT here}\vspace{1.5cm}}}
\end{center}
...
\dec{Confirm the three area thresholds against the home-range figures in Marker
et al. 2008 and Melzheimer et al. 2020 before locking. If the reported ranges
differ substantially from the assumption above, adjust the thresholds and record
the source. Also confirm that the archived dataset uses the evidence categories
named here; if the category labels differ, map them explicitly in an amendment.}
...
\dec{Confirm the acquisition order, and set a hard date by which the contingency
triggers.}
...
\dec{Confirm that RES-06 is added, and confirm RES-04 as the illustrative
surface. Both choices must be made now, not after seeing which surface produces
the clearest map.}
...
\dec{The \SI{100}{\kilo\metre} default is a placeholder pending a literature
check on reported cheetah natal dispersal distances. Confirm or replace it with
a cited value before locking, and record the source. If the literature does not
support a defensible central value, report the full sensitivity range and say
...
\dec{Confirm $k/n = 0.8$. This is the single most consequential number in the
protocol and the one most tempting to relax later. If you relax it after seeing
the agreement map, it must appear in the amendment log with the pre- and
...
\dec{Confirm the three weight sets. They must be fixed now. If a partner
organisation later supplies operational weights, those are added as a fourth set
in an amendment, not substituted for these.}
